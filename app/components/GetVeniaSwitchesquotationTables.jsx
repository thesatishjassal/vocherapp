      ...
      arr.forEach((it) => {
        ...
        Object.entries(it.qty).forEach(([variant, q]) => {
          if (q > 0) {
            ...
            const amount = q * mrp;                    // Gross Amount
            const net = amount * (1 - (it.discount_percent || 0) / 100); // Total Amount
            ...
            rows.push({
              ...
              color: variant === "dark_grey" ? "Dark Grey" : variant,
              category,
              qty: q,
              mrp,
              amount,                    // Gross
              discount: it.discount_percent || 0,
              netPrice: net,             // Total Amount after discount
              unit: "pcs",
              comments: "N/A",
            });
          }
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 15 }}>
        {[1, 2, 3].map((s) => (
          <div key={s} style={{ padding: "6px 12px", borderRadius: 6, fontWeight: 600, background: step === s ? "#ddd" : "#f2f2f2" }}>
            Step {s}
          </div>
        ))}
      </div>
              ...
              {!loading &&
                switchItems.map((sw) => (
                  <tr key={sw.item_code}>
                    ...
                    {/* <td>{sw.brand || "Wipro"}</td> */}
                    <td>{sw.model || "Venia"}</td>
                    <td>{sw.category || "Switch"}</td>
                    <td>{sw.white_mrp || 0}</td>
                    <td><input type="number" min="0" onChange={(e) => handleQtyChange(sw.item_code, "white", e.target.value)} /></td>
                    <td>{sw.dark_grey || 0}</td>
                    <td><input type="number" min="0" onChange={(e) => handleQtyChange(sw.item_code, "dark_grey", e.target.value)} /></td>
                    <td><input type="number" min="0" max="100" onChange={(e) => handleDiscountChange(sw.item_code, e.target.value)} /></td>
                  </tr>
                ))}
            </tbody>
          </table>
      ...
      <div style={{ marginTop: 20 }}>
        ...
        <button disabled={step === 5} onClick={() => setStep(step + 1)}>Next</button>
      </div>
